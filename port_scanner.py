...
def scan_udp_port(ip, port, timeout=2):
    ...
    try:
        ...
        banner = ""
        if data:
            try: banner = data[:80].decode('utf-8', errors='ignore').strip().replace('\n', ' ')
            except: pass
...
def interactive_mode():
    ...
    while True:
        ...
        # เลือกกลุ่ม port
        show_port_menu()
        port_choice = input(f"  {Colors.BOLD}เลือก [{Colors.CYAN}0-9{Colors.RESET}{Colors.BOLD}]: {Colors.RESET}").strip()
